...
회의 모드(`tst-meeting`)는 연속 녹음 → 청크 단위 전사 → 마크다운 파일 출력을 수행한다. [mdgate](https://github.com/cmygray/mdgate)와 함께 쓰면 전사 파일과 라이브 노트를 모바일에서 실시간으로 볼 수 있다.

Meeting mode (`tst-meeting`) continuously records audio, transcribes in chunks, and writes to a markdown file. Pair it with [mdgate](https://github.com/cmygray/mdgate) to view transcripts and live notes on your phone in real time.
...
tst 프로젝트에는 회의 모드를 자동 실행하는 `/meeting-start` 커맨드가 포함되어 있다 (`.claude/commands/meeting-start.md`). mdgate가 설치되어 있으면 `--share` 옵션으로 zrok 공개 URL을 자동 생성한다.
...
2. `tst-meeting` 백그라운드 실행 / Run `tst-meeting` in background
...
> mdgate 없이도 회의 모드는 정상 동작한다. 전사 파일은 `~/tst-meetings/`에 저장된다.
>
> Meeting mode works without mdgate. Transcripts are saved to `~/tst-meetings/`.
